...
ReviewStatus = Literal["all", "pending", "released", "partially-released", "rejected", "expired"]
...
def load_review_api_settings(environ=None):
    ...
    return ReviewApiSettings(
        repository_file=environ.get("NARROWCTI_QUARANTINE_REPOSITORY", os.path.join(state_dir, "quarantine.jsonl")),
        release_audit_file=environ.get("NARROWCTI_RELEASE_AUDIT_FILE", os.path.join(state_dir, "audit", "releases.jsonl")),
        credentials_file=environ.get("NARROWCTI_REVIEW_API_CREDENTIALS_FILE", ""),
        host=environ.get("NARROWCTI_REVIEW_API_HOST", "127.0.0.1"),
        port=int(environ.get("NARROWCTI_REVIEW_API_PORT", "8081")),
        docs_enabled=env_bool("NARROWCTI_REVIEW_API_DOCS_ENABLED", False, environ),
        allow_export=env_bool("NARROWCTI_REVIEW_API_ALLOW_EXPORT", False, environ),
        require_reason=env_bool("NARROWCTI_RELEASE_QUARANTINE_REQUIRES_REASON", True, environ),
        identity_name=environ.get("NARROWCTI_REVIEW_API_IDENTITY_NAME", "NarrowCTI Gateway"),
...
def build_export_dedup(settings, api_client):
    ...
    opencti_lookup = OpenCTIArtifactLookup(api_client) if settings.opencti_dedup_lookup else None
    if not local_index and not opencti_lookup:
        return None
    return CompositeArtifactDeduplication(local_index=local_index, opencti_lookup=opencti_lookup)


def create_app(settings=None, review_service=None, credential_store=None, opencti_client_factory=None):
    ...
    app = FastAPI(
        title="NarrowCTI Analyst Review API", version="1.0.0-dev.0",
    ...
    def authenticated_principal(authorization: Annotated[HTTPAuthorizationCredentials | None, Depends(bearer)]):
        principal = None
        if authorization and authorization.scheme.lower() == "bearer":
            principal = credential_store.authenticate(authorization.credentials)
        if not principal:
            raise HTTPException(status_code=401, detail="invalid or missing bearer credential", headers={"WWW-Authenticate": "Bearer"})
        return principal

    def require_permission(permission):
        def dependency(principal: Annotated[ReviewPrincipal, Depends(authenticated_principal)]):
    ...
    def summary(principal: Annotated[ReviewPrincipal, Depends(read_principal)]):
        del principal
        with repository_lock:
            return review_service.summary().to_dict()

    @app.get("/api/v1/review/records")
    def list_records(principal: Annotated[ReviewPrincipal, Depends(read_principal)], status: ReviewStatus = "pending", source_key: Annotated[str, Query(max_length=128)] = "", limit: Annotated[int, Query(ge=1, le=500)] = 100):
        del principal
        with repository_lock:
            records = review_service.list_records(status=status, source_key=source_key, limit=limit)
        return {"count": len(records), "items": [present_record(record, False, None) for record in records]}

    @app.get("/api/v1/review/records/{quarantine_id}")
    def get_record(quarantine_id: str, principal: Annotated[ReviewPrincipal, Depends(read_principal)], include_raw: bool = False):
        return present_record(guarded(lambda: review_service.get_record(quarantine_id)), include_raw, principal)

    @app.get("/api/v1/review/audit")
    def audit_events(principal: Annotated[ReviewPrincipal, Depends(read_principal)], quarantine_id: Annotated[str, Query(max_length=128)] = "", action: Annotated[str, Query(max_length=64)] = "", limit: Annotated[int, Query(ge=1, le=500)] = 100):
        del principal
        with repository_lock:
            events = review_service.audit_events(quarantine_id=quarantine_id, action=action, limit=limit)
        return {"count": len(events), "items": events}

    @app.post("/api/v1/review/records/{quarantine_id}/release")
    def release(quarantine_id: str, request: DecisionRequest, principal: Annotated[ReviewPrincipal, Depends(decision_principal)]):
        return guarded(lambda: review_service.release(quarantine_id, request.reason, reviewer=principal.principal))

    @app.post("/api/v1/review/records/{quarantine_id}/release-indicators")
    def release_indicators(quarantine_id: str, request: PartialReleaseRequest, principal: Annotated[ReviewPrincipal, Depends(decision_principal)]):
        return guarded(lambda: review_service.release_indicators(quarantine_id, request.indicator_types, request.reason, reviewer=principal.principal))

    @app.post("/api/v1/review/records/{quarantine_id}/reject")
    def reject(quarantine_id: str, request: DecisionRequest, principal: Annotated[ReviewPrincipal, Depends(decision_principal)]):
        return guarded(lambda: review_service.reject(quarantine_id, request.reason, reviewer=principal.principal))

    @app.post("/api/v1/review/records/{quarantine_id}/export-preview")
    def export_preview(quarantine_id: str, principal: Annotated[ReviewPrincipal, Depends(preview_principal)]):
        del principal
        return present_results(guarded(lambda: review_service.export_released(quarantine_id, dry_run=True)))
    ...
    def export(quarantine_id: str, principal: Annotated[ReviewPrincipal, Depends(export_principal)]):
        ...
        response = present_results(guarded(lambda: review_service.export_released(
            quarantine_id, api_client=api_client, artifact_dedup=dedup,
            identity_name=settings.identity_name, dry_run=False,
            exported_by=f"review-api:{principal.principal}", exporter=send_bundle,
        )))
...
__all__ = ["ReviewApiSettings", "DecisionRequest", "PartialReleaseRequest", "env_bool", "load_review_api_settings", "default_opencti_client_factory", "build_export_dedup", "create_app", "main"]
