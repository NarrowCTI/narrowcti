"""Bearer authentication for the analyst review API."""
...
class ReviewCredentialStore:
    ...
    def authenticate(self, token):
        ...
        return ReviewPrincipal(
            principal=matched["principal"], roles=matched["roles"],
...
def normalize_credentials(credentials):
    ...
    for index, credential in enumerate(credentials):
        ...
        credential_id = str(credential.get("credential_id") or f"credential-{index + 1}").strip()
        ...
        normalized.append({
            "principal": principal, "credential_id": credential_id,
            "token_sha256": token_hash, "roles": normalized_roles,
        })
    return normalized
...
__all__ = [
    "VALID_ROLES", "ROLE_PERMISSIONS", "ReviewPrincipal", "ReviewCredentialStore",
    "normalize_credentials", "token_sha256", "generate_token", "main",
]
