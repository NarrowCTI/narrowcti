...
class QuarantineExportResult:
    ...
    def to_dict(self):
        return {
            "quarantine_id": self.quarantine_id, "status": self.status,
            "action": self.action, "reason": self.reason,
            "indicator_count": self.indicator_count,
            "exported_indicator_count": self.exported_indicator_count,
            "dedup_duplicate_count": self.dedup_duplicate_count,
            "title": self.title, "source_key": self.source_key,
            "external_id": self.external_id, "dry_run": self.dry_run,
...
class QuarantineExporter:
    ...
    def export_record(self, record):
        ...
        if status not in EXPORTABLE_STATUSES:
            return QuarantineExportResult(**base, action="skip", reason=f"status is not exportable: {status}")
        review = record.get("review") or {}
        if review.get("exported"):
            return QuarantineExportResult(**base, action="skip", reason="already exported")
        indicators = released_indicators(record)
        if not indicators:
            return QuarantineExportResult(**base, action="skip", reason="no released indicators")
        indicators, duplicate_count = self.filter_new_indicators(record, indicators)
        if not indicators:
            if not self.dry_run:
                self.repository.mark_exported(
                    record["quarantine_id"], exported_indicator_count=0,
                    dedup_duplicate_count=duplicate_count, exported_by=self.exported_by,
                )
            return QuarantineExportResult(
                **base, action="dry-run" if self.dry_run else "dedup-skip",
                reason="all released indicators already known",
                indicator_count=len(released_indicators(record)),
                exported_indicator_count=0, dedup_duplicate_count=duplicate_count,
            )
        if self.dry_run:
            return QuarantineExportResult(
                **base, action="dry-run", reason="would export released quarantine record",
                indicator_count=len(indicators), exported_indicator_count=len(indicators),
                dedup_duplicate_count=duplicate_count,
            )
        if self.exporter is None:
            return QuarantineExportResult(
                **base, action="error", reason="export operation is not configured",
                indicator_count=len(indicators), dedup_duplicate_count=duplicate_count,
                errors=("export operation is not configured",),
            )
        try:
            exported_count = self.exporter(
                self.api_client, record_title(record), record_description(record),
                record_score(record), indicators, identity_name=self.identity_name,
            )
        except Exception as exc:
            self.logger(f"Quarantine export failed: id={record.get('quarantine_id')} error={exc}")
            return QuarantineExportResult(
                **base, action="error", reason="export failed", indicator_count=len(indicators),
                dedup_duplicate_count=duplicate_count, errors=(str(exc),),
            )
        self.mark_artifacts(record, indicators)
        self.repository.mark_exported(
            record["quarantine_id"], exported_indicator_count=exported_count,
            dedup_duplicate_count=duplicate_count, exported_by=self.exported_by,
        )
        self.logger(
        ...
        return QuarantineExportResult(
            **base, action="export", reason="exported released quarantine record",
            indicator_count=len(indicators), exported_indicator_count=exported_count,
            dedup_duplicate_count=duplicate_count,
        )
    ...
    def filter_new_indicators(self, record, indicators):
        if not self.artifact_dedup:
            return indicators, 0
        filtered, duplicate_count = self.artifact_dedup.filter_new_indicators(indicators)
        if duplicate_count:
            self.logger(
    ...
    def mark_artifacts(self, record, indicators):
        ...
        try:
            added = self.artifact_dedup.mark_indicators(
                indicators, source_key=record.get("source_key", ""),
...
def exportable_records(records, quarantine_id=""):
    return [
        ...
        if (not quarantine_id or record.get("quarantine_id") == quarantine_id)
        and normalize_status(record.get("status")) in EXPORTABLE_STATUSES
    ]
...
def result_base(record, dry_run=False):
    return {
        ...
        "status": record.get("status", ""), "title": record_title(record),
        "source_key": record.get("source_key", ""),
        "external_id": record.get("external_id", ""), "dry_run": dry_run,
    }
