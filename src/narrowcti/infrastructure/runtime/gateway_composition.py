"""Concrete Community source composition for the gateway CLI."""
...
def build_source_dedup(api_client, artifact_dedup, opencti_dedup_lookup, logger):
    if not artifact_dedup and not opencti_dedup_lookup:
        return None
    lookup = OpenCTIArtifactLookup(api_client, logger=logger) if opencti_dedup_lookup else None
...
def gateway_file(gateway_settings, directory_name, filename):
    ...
    return os.path.join(base_dir, filename) if base_dir else ""


def apply_gateway_source_paths(settings, gateway_settings, source_key, environ: Mapping[str, str] | None = None):
    ...
    if path_config["state_env"] not in env:
        updates["state_file"] = gateway_file(gateway_settings, "state_dir", path_config["state_file"])
    if path_config["audit_env"] not in env:
        updates["decision_audit_file"] = gateway_file(gateway_settings, "decision_audit_dir", path_config["audit_file"])
    if path_config["quarantine_env"] not in env and "NARROWCTI_QUARANTINE_REPOSITORY" not in env:
        updates["quarantine_repository_file"] = getattr(gateway_settings, "quarantine_repository_file", "")
    return replace(settings, **updates) if updates else settings


def build_otx_runner(logger, artifact_dedup=None, opencti_dedup_lookup=False, gateway_settings=None):
...
def build_misp_runner(logger, artifact_dedup=None, opencti_dedup_lookup=False, gateway_settings=None):
    ...
    adapter = MISPFeedAdapter(misp, limits=settings.adapter_limits, logger=logger, search_filters=settings.search_filters)
...
def build_artifact_dedup(gateway_settings):
    if not gateway_settings or gateway_settings.dedup_mode not in ["artifact", "hybrid"]:
        return None
...
def default_source_registry(logger, gateway_settings=None):
    ...
    opencti_dedup_lookup = bool(gateway_settings and gateway_settings.opencti_dedup_lookup)
    return (
        SourceRegistry()
        .register("otx", "OTX", lambda: build_otx_runner(logger, artifact_dedup, opencti_dedup_lookup, gateway_settings))
        .register("misp", "MISP", lambda: build_misp_runner(logger, artifact_dedup, opencti_dedup_lookup, gateway_settings))
    )


__all__ = [
    "SOURCE_RUNTIME_PATHS", "ProcessorRunner", "build_source_dedup", "gateway_file",
