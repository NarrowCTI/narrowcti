...
def send_bundle(
    ...
    # Keep monkey-patching of the historical compatibility surface observable
    # while the exporter implementation is owned by this canonical module.
    compatibility_module = sys.modules.get("exporters.opencti")
    capture_worker_errors = getattr(
        compatibility_module,
        "capture_pycti_worker_errors",
        capture_pycti_worker_errors,
    )
    worker_errors, restore_worker_logger = capture_worker_errors(api_client)
    try:
