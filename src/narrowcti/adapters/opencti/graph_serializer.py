...
def build_graph_content(accepted_candidates, identity_id, now, external_object_ids=None):
    semantic_compilation = compile_graph_semantics(
        accepted_candidates,
        key_resolver=lambda candidate, _index: graph_object_key(candidate),
        existing_reference_resolver=existing_opencti_ref,
        deduplicate=False,
    )
    semantic_by_key = {}
    for semantic_object in semantic_compilation.objects:
        semantic_by_key.setdefault(semantic_object.key, semantic_object)
    graph_objects = []
    graph_object_ids = {}
    ...
    for candidate in accepted_candidates:
        ...
        try:
            ...
            stix_object = None
        semantic_object = semantic_by_key.get(object_key)
        existing_ref = existing_opencti_ref(candidate)
        if semantic_object is not None and semantic_object.existing_reference:
            existing_ref = semantic_object.existing_reference
        if existing_ref:
