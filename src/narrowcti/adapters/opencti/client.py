"""OpenCTI client with the existing version-scoped compatibility behavior."""
...
PYCTI7_INPUT_FIELDS = frozenset({"AIPrompt", "ICCID", "IMEI", "IMSI", "embedded", "files", "filesMarkings", "noTriggerImport", "upsertOperations"})
PYCTI7_QUERY_INPUTS = ("AIPrompt", "ICCID", "IMEI", "IMSI", "upsertOperations")
        ...
        for key, item in value.items():
            ...
            if key in PYCTI7_INPUT_FIELDS:
                if not _is_empty_compatibility_value(item):
                    raise OpenCTICompatibilityError(f"OpenCTI 6.9 compatibility cannot discard {item_path}")
                continue
            sanitized[key] = sanitize_legacy_variables(item, item_path)
        return sanitized
    if isinstance(value, list):
        return [sanitize_legacy_variables(item, f"{path}[{index}]") for index, item in enumerate(value)]
...
def sanitize_legacy_query(query):
    ...
    for line in str(query or "").splitlines(keepends=True):
        if any(f"${input_name}:" in line or f"{input_name}: ${input_name}" in line for input_name in PYCTI7_QUERY_INPUTS):
            continue
        lines.append(line)
    return "".join(lines)
...
class NarrowCTIOpenCTIApiClient(OpenCTIApiClient):
    ...
    def health_check(self):
        try:
            ...
            self.platform_version = str(((result.get("data") or {}).get("about") or {}).get("version") or "").strip()
...
__all__ = [
    ...
    "OpenCTICompatibilityError", "sanitize_legacy_variables", "sanitize_legacy_query",
    "NarrowCTIOpenCTIApiClient", "build_opencti_client",
]
