...
| Detection rule | `indicator` or `note` | Exportable as pattern-aware Indicator for OpenCTI-compatible YARA/Sigma evidence. Snort, Suricata and PCRE pass through a compatibility gate and are preserved as labeled Notes with source references and raw rule content until OpenCTI-compatible Indicator validation exists. |
