...
| Indicators | Creates indicators with observable-derived patterns and score metadata. | Creates simple STIX indicators for selected IoC types; YARA, Sigma, Snort, Suricata and PCRE attributes are parsed into audit-only `detection_rule` candidates. | Preserve official pattern semantics plus NarrowCTI score evidence. |
...
| Relationships | Creates `based-on`, `indicates`, `related-to`, `uses`, sightings and object-reference links. | Relationship export is not emitted by the current exporter; attribute sightings and object references are parsed into audit-only `sighting` and `object_reference` candidates. | Add relationship policy with confidence and provenance. |
