...
| Relationships | Creates `based-on`, `indicates`, `related-to`, `uses`, sightings and object-reference links. | Relationship export is not emitted by the current exporter; attribute sightings are parsed into audit-only `sighting` candidates. | Add relationship policy with confidence and provenance. |
