...
| Observations | Indicators | `indicator` | Validated export | Legacy indicator export remains active. Detection rules materialize as pattern-aware Indicators with canonical names such as `SIGMA: Suspicious PowerShell`, STIX `indicator_types=malicious-activity`, labels such as `narrowcti:detection-rule` and `rule-type:sigma`, source external references and source-backed descriptions. Real MISP validation confirmed YARA Indicators from event `849`, a Sigma Indicator from event `1027`, and YARA/Sigma Indicators from event `1649`, linked back to source context with author `MISP via NarrowCTI`. NarrowCTI validates Sigma structure before export; malformed Sigma from event `849` was intentionally excluded after OpenCTI rejected it as incorrectly formatted. Real event `1649` exposed the remaining compatibility gap: OpenCTI 6.9.4 rejected Snort Indicator creation with `INCORRECT_INDICATOR_FORMAT`, and also rejected some Sigma rules despite basic YAML validity. Snort, Suricata and PCRE must therefore remain audit/polish targets until OpenCTI-compatible validation or an alternate evidence representation is implemented. | Add stricter OpenCTI-compatible detection-rule validation and avoid promising materialized Indicators for rule types OpenCTI rejects. Continue Suricata and PCRE real-source validation when usable events are available. |
| Observations | Infrastructures | `infrastructure` | Validated export | Curated Infrastructure export is active for source-backed infrastructure. Validations cover actor infrastructure, MISP infrastructure objects and ASN/IP/CIDR relationships. Real MISP event `1649` materialized Infrastructure `MISP ip-port 137.184.181.252` with author `MISP via NarrowCTI`, linked it to IPv4 observable `137.184.181.252` with `consists-of`, and validated five IPv4 `belongs-to` ASN relationships to `DIGITALOCEAN-ASN` and `BL Networks`. Unit-level STIX builder coverage also confirms `IPv6-Addr -> belongs-to -> Autonomous-System` direction. Raw IP indicators are intentionally not promoted into Infrastructure by themselves. | Polish Infrastructure view completeness: attach ASN context to the primary Infrastructure IP when source/enrichment supports it, validate domain-ip and netblock examples with real feeds, and verify how OpenCTI displays the Infrastructure -> IP -> ASN chain in the UI after clean reloads. |
...
| Knowledge | Semantic relationships | `relationship` | Validated export where endpoints resolve | ObjectReference and curated graph relationships export only when both endpoints are resolvable. Real MISP `event:1649` confirmed source-backed Infrastructure relationships such as `Infrastructure -> consists-of -> IPv4-Addr` and `IPv4-Addr -> belongs-to -> Autonomous-System`. Real MISP `event:5379` confirmed that Report-only victimology context materializes as Report `related-to` edges even when NarrowCTI proposes `targets`; this is intentional until a real semantic source endpoint exists. | Continue strict endpoint resolution and relationship type validation. Add source-anchored semantic edges for actor/campaign/malware/infrastructure/victimology when the feed payload provides both endpoints. |
...
1. Expand real feed validation for OTX and MISP payloads that carry actor,
   ...
   endpoint for semantic `targets` relationships.
2. Validate expanded source-backed Campaign mapping. MISP Galaxy, explicit MISP
   ...
   live source-payload evidence. Campaign-backed target evidence is the
   preferred next path for `Campaign -> targets -> Organization/Sector/Country`
   semantic edges.
...
5. Continue detection-rule polish beyond the real MISP YARA/Sigma evidence.
   ...
   YAML validity. Remaining work is stricter OpenCTI-compatible validation,
   Suricata/PCRE real evidence and a product decision for preserving rejected
   rule content as analyst evidence.
