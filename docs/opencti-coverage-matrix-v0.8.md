...
| Locations | Countries | `location` | Validated export | Country lookup and deterministic export were validated with `Argentina`; repeat exports referenced the existing object. Conservative country normalization now maps common source aliases and ISO-style values such as `AR` -> `Argentina`, `BR` -> `Brazil` and `US` -> `United States` while preserving the original feed value in provenance. | Expand the controlled country alias dictionary with real source evidence and validate deeper country/region hierarchy behavior. |
