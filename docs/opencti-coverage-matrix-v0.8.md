...
| Techniques | Narratives | OpenCTI Narrative entity | Validated export | Controlled export is active for explicit source-backed `narrative` candidates. NarrowCTI emits OpenCTI custom SDOs with the required `extension-definition`, preserves aliases and `narrative_types`, and queries `narratives` before creation to avoid duplicates. Live validation imported `NarrowCTI Matrix Narrative Builder Validation 20260626C` as a Narrative with `objective` type. MISP Galaxy meta aliases such as `objective`, `motivation`, `theme`, `goal`, `intent`, `observed_motivations` and `cfr-type-of-incident`, plus explicit OTX fields such as `objectives`, `goals`, `motivations` and `narratives`, now create controlled Narrative candidates when the value is not IOC-like. Real MISP exports validated `Espionage` from `Galaxy.meta.cfr-type-of-incident` and `Financial Gain` from `Galaxy.meta.observed_motivations`; OpenCTI API returned both as Narratives authored by `MISP via NarrowCTI`. Narrative-like evidence still remains in Reports/Notes unless a source field is precise enough for first-class Narrative promotion. | Continue validating objective/theme/intent payloads from real feeds; do not promote generic report prose as Narrative. |
...
| Locations | Countries | `location` | Validated export | Country lookup and deterministic export were validated with `Argentina`; repeat exports referenced the existing object. Conservative country normalization now maps common source aliases and ISO-style values such as `AR` -> `Argentina`, `BR` -> `Brazil` and `US` -> `United States` while preserving the original feed value in provenance. Structured MISP/OTX victimology geography now receives source-specific confidence weighting. Explicit MISP `target-location` dry-run validation extracted Belgium, Sweden, Russia and China from real `event:280`; the event was not ingested under default policy because it scored `50`. Bounded real MISP exports then validated CFR victimology countries from `event:1578` and explicit `target-location` countries from `event:1150`; OpenCTI API returned Countries such as `Ukraine`, `Iran` and `Russia` authored by `MISP via NarrowCTI`. | Expand the controlled country alias dictionary with real source evidence and validate parent hierarchy behavior when feeds carry region, administrative-area, city or coordinate context. |
...
| Observations | Indicators | `indicator` | Validated export | Legacy indicator export remains active. Detection rules materialize as pattern-aware Indicators with canonical names such as `SIGMA: Suspicious PowerShell`, STIX `indicator_types=malicious-activity`, labels such as `narrowcti:detection-rule` and `rule-type:sigma`, source external references and source-backed descriptions when the rule type is OpenCTI-compatible. Real MISP validation confirmed YARA Indicators from event `849`, a Sigma Indicator from event `1027`, and YARA/Sigma Indicators from event `1649`, linked back to source context with author `MISP via NarrowCTI`. NarrowCTI now applies a stricter Sigma compatibility gate before Indicator export: Sigma rules need title, logsource, detection mapping, condition and at least one detection selection. Sigma rules that fail this gate are preserved as labeled Notes with the raw rule and compatibility reason instead of being discarded or retried through native Indicator creation. Snort, Suricata and PCRE also pass through the Note compatibility gate until OpenCTI-compatible Indicator validation exists for those rule types. Native GraphQL Indicator creation is limited to Sigma rules that pass the compatibility gate. Follow-up real OpenCTI API validation on MISP `event:1649` confirmed Sigma Indicators with detection-rule labels and four Snort rules preserved as Notes instead of native Indicators. A later source-shape validation found three real MISP `suricata` objects in the same event, with rule attributes stored as MISP type `snort` but `object_relation=suricata`; the parser now classifies those as `rule-type:suricata`, and post-export API validation confirmed four Suricata Notes in OpenCTI. | Validate PCRE when a real source payload is available and keep repeated historical replay relationship warnings in polish. |
| Observations | Infrastructures | `infrastructure` | Validated export | Curated Infrastructure export is active for source-backed infrastructure. Validations cover actor infrastructure, MISP infrastructure objects, ASN/IP/CIDR relationships and same-event MISP Galaxy context. Real MISP event `1649` materialized Infrastructure `MISP ip-port 137.184.181.252` with author `MISP via NarrowCTI`, linked it to IPv4 observable `137.184.181.252` with `consists-of`, and validated five IPv4 `belongs-to` ASN relationships to `DIGITALOCEAN-ASN` and `BL Networks`. A follow-up real export of the same event validated direct `Malware -> uses -> Infrastructure` relationships for `Lorenz`, `Lorenz Ransomware`, `Chisel (ELF)` and `Chisel (Windows)`, plus direct `Infrastructure -> related-to -> ATT&CK Attack Pattern` relationships for Kill Chain context. Top-level MISP infrastructure attributes `ip-src|port`, `ip-dst|port`, `hostname|port`, `domain|ip`, `AS` and `asn` now reuse the same safe normalization path as official MISP infrastructure objects. Real MISP `event:5280` validated 49 `domain|ip` Infrastructures and then direct `Intrusion-Set APT32 -> uses -> Infrastructure` relationships for those domains after MISP Galaxy tag fallback was enabled. Real MISP `event:1534` validated two `ip-src|port` Infrastructures, `MISP ip-port 196.53.114.199` and `MISP ip-port 204.11.49.132`, each linked to an IPv4 observable with port `80`; OpenCTI API returned both Infrastructures and both IPv4 observables. Unit-level STIX builder coverage also confirms `IPv6-Addr -> belongs-to -> Autonomous-System` direction. Offline MISP IP-to-ASN enrichment now emits both `IP -> belongs-to -> ASN` and `Infrastructure -> consists-of -> ASN` when an explicit Infrastructure object anchors the IP. Raw IP indicators are intentionally not promoted into Infrastructure by themselves. | Validate how OpenCTI displays the enriched Infrastructure -> IP -> ASN, adversary, capability and ATT&CK paths in Infrastructure, Knowledge, Diamond and graph views after clean reloads. Victimology from Infrastructure remains held until explicit source-backed `targets` behavior is validated. |
...
| Analyses | Notes | `note` | Validated export | Detection guidance and MISP EventReports can export as Notes. Detection-rule compatibility Notes now preserve Snort, Suricata, PCRE and Sigma rules that fail the OpenCTI Indicator compatibility gate, with raw rule content, compatibility reason, `narrowcti:detection-rule` and `rule-type:*` labels. Real MISP `event:1649` validation confirmed four Snort Notes authored by `MISP via NarrowCTI` with `rule-type:snort` labels. The same event now also validates Suricata Notes: the rebuilt image classifies the event's Suricata object rules as `rule-type:suricata`, and OpenCTI API validation returned four `SURICATA:` Notes authored by `MISP via NarrowCTI`. | Expand relationship targets for notes when the source context is precise and validate PCRE Notes when a source feed provides them. |
| Knowledge | Semantic relationships | `relationship` | Validated export where endpoints resolve | ObjectReference and curated graph relationships export only when both endpoints are resolvable. Real MISP `event:1649` confirmed source-backed Infrastructure relationships such as `Infrastructure -> consists-of -> IPv4-Addr` and `IPv4-Addr -> belongs-to -> Autonomous-System`; follow-up export confirmed `Malware -> uses -> Infrastructure` and `Infrastructure -> related-to -> Attack Pattern` relationships directly on the same Infrastructure object. Real MISP `event:5280` confirmed `Intrusion-Set APT32 -> uses -> Infrastructure` relationships for domain/IP Infrastructure after MISP Galaxy tag fallback. Real MISP `event:5379` confirmed that Report-only victimology context materializes as Report `related-to` edges even when NarrowCTI proposes `targets`; this is intentional until a real semantic source endpoint exists. Follow-up unit validation now proves explicit MISP victimology can become semantic `targets`, and MISP Galaxy arsenal/ATT&CK can become semantic `uses`, when one trusted campaign/actor/intrusion-set anchor is present and unambiguous. | Continue strict endpoint resolution and relationship type validation. Validate source-anchored semantic edges with real actor/campaign/malware/infrastructure/victimology payloads, especially Infrastructure victimology before enabling it broadly. |
...
1. Continue real feed validation for source diversity rather than core MISP
   coverage. Events `1649`, `1578`, `5559`, `7`, `1534`, `1150`, `1442` and
   `5280` now cover rich Infrastructure/IP/ASN/Malware/CVE/ATT&CK/victimology,
   source-anchored actor/intrusion-set to ATT&CK/victimology, explicit
   ...
   available.
2. Campaign mapping is validated for explicit MISP `campaign-name` through
   `event:7`. Keep validating campaign-backed semantic `targets` when a source
   carries Campaign plus Organization/Sector/Country in the same unambiguous
...
4. Channels, Events, Security Platforms and Systems remain feed-dependent for
   ...
   the additional local feed pass, MISP still does not carry structured channel,
   security-platform, human system or discrete CTI-event fields; broad keyword
   hits were only references or prose and are intentionally not promoted.
   Narratives are no longer blocked: real MISP exports validated `Espionage`
   and `Financial Gain`.
5. Continue detection-rule polish beyond the real MISP YARA/Sigma/Snort
   evidence.
   ...
   confirmed Snort Notes from MISP `event:1649`. A later source-shape pass
   found real Suricata MISP objects in the same event, fixed their
   classification from Snort to Suricata and confirmed four Suricata Notes
   through the OpenCTI API. Remaining work is PCRE real evidence, broader UI
   confirmation that detection-rule Notes give analysts enough hunting context
   without polluting Indicator views and repeated historical replay polish for
   stale Indicator relationship warnings. The current local MISP dataset still
