...
| Observations | Indicators | `indicator` | Validated export | Legacy indicator export remains active. Detection rules materialize as pattern-aware Indicators with canonical names such as `SIGMA: Suspicious PowerShell`, STIX `indicator_types=malicious-activity`, labels such as `narrowcti:detection-rule` and `rule-type:sigma`, source external references and source-backed descriptions when the rule type is OpenCTI-compatible. Real MISP validation confirmed YARA Indicators from event `849`, a Sigma Indicator from event `1027`, and YARA/Sigma Indicators from event `1649`, linked back to source context with author `MISP via NarrowCTI`. NarrowCTI validates Sigma structure before export; malformed Sigma from event `849` was intentionally excluded after OpenCTI rejected it as incorrectly formatted. Real event `1649` exposed the remaining compatibility gap: OpenCTI 6.9.4 rejected Snort Indicator creation with `INCORRECT_INDICATOR_FORMAT`, and also rejected some Sigma rules despite basic YAML validity. Snort, Suricata and PCRE now pass through a compatibility gate that preserves their raw rule content as labeled Notes instead of promising materialized Indicators. Native GraphQL Indicator creation is limited to Sigma until more OpenCTI-compatible rule types are validated. | Add stricter OpenCTI-compatible detection-rule validation for Sigma variants and validate Suricata/PCRE real-source payloads when usable events are available. |
...
| Analyses | Notes | `note` | Validated export | Detection guidance and MISP EventReports can export as Notes. Detection-rule compatibility Notes now preserve Snort, Suricata and PCRE raw rule content with `narrowcti:detection-rule` and `rule-type:*` labels when OpenCTI Indicator materialization is not trusted. | Expand relationship targets for notes when the source context is precise and validate detection-rule Notes in real OpenCTI UI/API runs. |
...
5. Continue detection-rule polish beyond the real MISP YARA/Sigma evidence.
   ...
   as labeled Notes instead of native Indicators. Remaining work is stricter
   OpenCTI-compatible validation for Sigma variants, Suricata/PCRE real
   evidence and UI/API confirmation that detection-rule Notes give analysts
   enough hunting context without polluting Indicator views.
