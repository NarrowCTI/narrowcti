...
| Arsenal | Channels | OpenCTI Channel entity | Validated export | Controlled export is active for explicit source-backed `channel` candidates. NarrowCTI emits OpenCTI custom SDOs with the required `extension-definition`, preserves aliases and `channel_types`, and queries `channels` before creation to avoid duplicates. Live validation imported `NarrowCTI Matrix Channel Builder Validation 20260626C` as a Channel with `c2` and `delivery` types. | Map only source-backed C2, marketplace, communication or delivery channel fields from real OTX/MISP/future payloads. Do not infer channels from report titles alone. |
...
4. Expand real source mapping for Channels, Narratives and Events. Controlled
   OpenCTI import behavior is now proven; the remaining work is source-payload
   evidence and conservative promotion policy.
