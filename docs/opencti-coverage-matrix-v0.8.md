...
| Analyses | Notes | `note` | Validated export | Detection guidance and MISP EventReports can export as Notes. Detection-rule compatibility Notes now preserve Snort, Suricata, PCRE and Sigma rules that fail the OpenCTI Indicator compatibility gate, with raw rule content, compatibility reason, `narrowcti:detection-rule` and `rule-type:*` labels. | Expand relationship targets for notes when the source context is precise and validate detection-rule Notes in real OpenCTI UI/API runs. |
...
5. Continue detection-rule polish beyond the real MISP YARA/Sigma evidence.
   ...
   Notes instead of native Indicators. Remaining work is Suricata/PCRE real
