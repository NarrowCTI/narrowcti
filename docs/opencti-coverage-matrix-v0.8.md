...
| Arsenal | Channels | OpenCTI Channel entity | Validated export | Controlled export is active for explicit source-backed `channel` candidates. NarrowCTI emits OpenCTI custom SDOs with the required `extension-definition`, preserves aliases and `channel_types`, and queries `channels` before creation to avoid duplicates. Live validation imported `NarrowCTI Matrix Channel Builder Validation 20260626C` as a Channel with `c2` and `delivery` types. MISP Galaxy meta aliases such as `c2-channel`, `communication-channel`, `delivery-channel` and `marketplace` now create controlled Channel candidates when the value is not IOC-like. | Validate the new MISP Galaxy meta channel mapping with real payloads. Do not infer channels from report titles alone. |
...
4. Expand real source mapping for Channels, Narratives, Events, Security
   Platforms and Systems. MISP Galaxy meta aliases are now implemented with
   IOC/provenance guardrails and covered by unit tests; the remaining work is
   live source-payload evidence and conservative promotion policy tuning.
