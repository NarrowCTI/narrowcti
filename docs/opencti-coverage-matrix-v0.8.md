...
| Locations | Countries | `location` | Validated export | Country lookup and deterministic export were validated with `Argentina`; repeat exports referenced the existing object. Conservative country normalization now maps common source aliases and ISO-style values such as `AR` -> `Argentina`, `BR` -> `Brazil` and `US` -> `United States` while preserving the original feed value in provenance. Structured MISP/OTX victimology geography now receives source-specific confidence weighting. | Expand the controlled country alias dictionary with real source evidence and validate deeper country/region hierarchy behavior. |
...
| Observations | Indicators | `indicator` | Validated export | Legacy indicator export remains active. Detection rules materialize as pattern-aware Indicators with canonical names such as `SIGMA: Suspicious PowerShell`, STIX `indicator_types=malicious-activity`, labels such as `narrowcti:detection-rule` and `rule-type:sigma`, source external references and source-backed descriptions. Real MISP validation confirmed YARA Indicators from event `849` and a Sigma Indicator from event `1027`, linked back to the source Report with author `MISP via NarrowCTI`. NarrowCTI validates Sigma structure before export; malformed Sigma from event `849` was intentionally excluded after OpenCTI rejected it as incorrectly formatted. This keeps YARA, Sigma, Snort, Suricata and PCRE evidence in the OpenCTI Indicator workflow without inventing a non-standard detection-rule SDO. | Expand real Snort, Suricata and PCRE validation when usable source events are available. |
| Observations | Infrastructures | `infrastructure` | Validated export | Curated Infrastructure export is active for source-backed infrastructure. Validations cover actor infrastructure, MISP infrastructure objects and ASN/IP/CIDR relationships. Unit-level STIX builder coverage now also confirms `IPv6-Addr -> belongs-to -> Autonomous-System` direction. Raw IP indicators are intentionally not promoted into Infrastructure by themselves. | Expand true external MISP/OTX payload validation for ASN, netblock, domain-ip and ip-port evidence, and verify source-backed IPv4/IPv6 attachment visually after the clean OpenCTI reload. |
...
5. Continue detection-rule polish beyond the real MISP YARA/Sigma evidence.
   ...
   actor and victimology context. Remaining work is broader Snort, Suricata and
   PCRE source evidence plus UI review of analyst usefulness.
